...
describe("removeProps functions", () => {
  ...
  it("should remove Grid.HorizontalSize props", () => {
    ...
    expect(
      Grid.removeGridSizesProps({ ...props, ...horizontalSizeProps })
    ).toEqual(props);
  });
  ...
  it("should remove Grid.Size props", () => {
    ...
    expect(Grid.removeSizeProps({ ...props, ...sizeProps })).toEqual(props);
  });
  ...
  it("should remove Grid.Offset props", () => {
    ...
    expect(Grid.removeOffsetProps({ ...props, ...offsetProps })).toEqual(props);
  });
});
...
describe("get*Modifiers functions", () => {
  it("should getSizeModifiers", () => {
    ...
    expect(Grid.getSizeModifiers({ isSize: "1/2" })).toEqual({
      "is-half": true
    });
    expect(Grid.getSizeModifiers({ isSize: "narrow" })).toEqual({
      "is-narrow": true
    });

    expect(
      Grid.getSizeModifiers({
        isSize: { mobile: "narrow", s: 12, desktop: 10 }
      })
    ...
    expect(
      Grid.getSizeModifiers({
        isSize: { touch: "narrow", s: 12, fullhd: "narrow" }
      })
  ...
  it("should getOffsetModifiers", () => {
    expect(Grid.getOffsetModifiers({ isOffset: 1 })).toEqual({
      "is-offset-1": true
    });
    expect(Grid.getOffsetModifiers({ isOffset: 12 })).toEqual({
      "is-offset-12": true
    });

    expect(Grid.getOffsetModifiers({ isOffset: "1/2" })).toEqual({
      "is-offset-half": true
    });
    expect(Grid.getOffsetModifiers({ isOffset: "narrow" })).toEqual({});

    expect(
      Grid.getOffsetModifiers({
        isOffset: { mobile: "narrow", s: 12, desktop: 10 }
      })
    ).toEqual({ "is-offset-10-desktop": true });
    expect(
      Grid.getOffsetModifiers({
        isOffset: { mobile: "narrow", s: 12, touch: 10 }
      })
    ).toEqual({ "is-offset-10-touch": true });
    expect(
      Grid.getOffsetModifiers({
        isOffset: { mobile: "narrow", s: 12, fullhd: 10 }
      })
